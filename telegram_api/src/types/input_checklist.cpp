...
InputChecklist Parse(const Value& value, To<InputChecklist>) {
    ...
    obj.others_can_add_tasks = ParseComplex<Optional<Boolean>>(value["others_can_add_tasks"]);
    obj.others_can_mark_tasks_as_done = ParseComplex<Optional<Boolean>>(value["others_can_mark_tasks_as_done"]);
    return obj;
}
