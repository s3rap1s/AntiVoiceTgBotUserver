...
BotCommandScope Parse(const Value& value, To<BotCommandScope>) {
    ...
    if (type == "all_private_chats") return ParseComplex<BotCommandScopeAllPrivateChats>(value);
    if (type == "all_group_chats") return ParseComplex<BotCommandScopeAllGroupChats>(value);
    if (type == "all_chat_administrators") return ParseComplex<BotCommandScopeAllChatAdministrators>(value);
