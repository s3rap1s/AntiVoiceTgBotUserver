...
TransactionPartner Parse(const Value& value, To<TransactionPartner>) {
    ...
    if (type == "affiliate_program") return ParseComplex<TransactionPartnerAffiliateProgram>(value);
    if (type == "fragment") return ParseComplex<TransactionPartnerFragment>(value);
