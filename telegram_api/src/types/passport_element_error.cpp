...
PassportElementError Parse(const Value& value, To<PassportElementError>) {
    ...
    if (source == "data") return ParseComplex<PassportElementErrorDataField>(value);
    if (source == "front_side") return ParseComplex<PassportElementErrorFrontSide>(value);
    if (source == "reverse_side") return ParseComplex<PassportElementErrorReverseSide>(value);
    if (source == "selfie") return ParseComplex<PassportElementErrorSelfie>(value);
    if (source == "file") return ParseComplex<PassportElementErrorFile>(value);
    if (source == "files") return ParseComplex<PassportElementErrorFiles>(value);
